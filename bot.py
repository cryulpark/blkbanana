import ccxt
import time
import telegram  # pip install python-telegram-bot
from telegram.ext import Application
import os

# API 키 환경 변수로만 읽기 (유출 방지)
try:
    BINANCE_API = os.environ['BINANCE_API_KEY']
    BINANCE_SECRET = os.environ['BINANCE_SECRET']
    UPBIT_API = os.environ['UPBIT_API_KEY']
    UPBIT_SECRET = os.environ['UPBIT_SECRET']
    BITHUMB_API = os.environ['BITHUMB_API_KEY']
    BITHUMB_SECRET = os.environ['BITHUMB_SECRET']
    BYBIT_API = os.environ['BYBIT_API_KEY']
    BYBIT_SECRET = os.environ['BYBIT_SECRET']
    OKX_API = os.environ['OKX_API_KEY']
    OKX_SECRET = os.environ['OKX_SECRET']
    TELEGRAM_TOKEN = os.environ['TELEGRAM_TOKEN']
    CHAT_ID = os.environ['CHAT_ID']
except KeyError as e:
    raise ValueError(f"환경 변수 누락: {e}")

# 거래소 연결 (개별 try로 안전)
exchanges = {}
for name, api, secret in [
    ('binance', BINANCE_API, BINANCE_SECRET),
    ('upbit', UPBIT_API, UPBIT_SECRET),
    ('bithumb', BITHUMB_API, BITHUMB_SECRET),
    ('bybit', BYBIT_API, BYBIT_SECRET),
    ('okx', OKX_API, OKX_SECRET)
]:
    try:
        exchanges[name] = getattr(ccxt, name)({'apiKey': api, 'secret': secret})
    except Exception as e:
        print(f"{name} 연결 오류: {e}")

app = Application.builder().token(TELEGRAM_TOKEN).build()

def send_telegram(message):
    try:
        app.bot.send_message(chat_id=CHAT_ID, text=message)
    except Exception as e:
        print(f"텔레그램 오류: {e}")

def get_exchange_rate(base_ex='binance'):
    try:
        return exchanges[base_ex].fetch_ticker('USDT/KRW')['bid']
    except Exception as e:
        send_telegram(f"환율 오류: {e}")
        return 1350  # fallback

def get_spread(base_ex='binance', target_ex='upbit', pair='BTC/USDT', krw_pair='BTC/KRW'):
    try:
        btc_base = exchanges[base_ex].fetch_ticker(pair)['bid']
        btc_target = exchanges[target_ex].fetch_ticker(krw_pair)['ask'] / get_exchange_rate()
        return (btc_target / btc_base - 1) * 100
    except Exception as e:
        send_telegram(f"{target_ex} 스프레드 오류: {e}")
        return 0

def get_funding_rate(ex='binance', pair='BTC/USDT'):
    try:
        return exchanges[ex].fetch_funding_rate(pair)['rate']
    except Exception as e:
        send_telegram(f"{ex} 펀딩 레이트 오류: {e}")
        return 0

# 메인 루프 (안전 우선, 꾸준 수익)
while True:
    try:
        spreads = {}
        for target in ['upbit', 'bithumb', 'bybit', 'okx']:
            spreads[target] = get_spread('binance', target)
        
        max_spread_ex = max(spreads, key=spreads.get)
        spread = spreads[max_spread_ex]
        if spread > 2.5:  # 안전 임계값
            amount = 0.001  # 10% 룰 준수 (2천만 원 중 200만 원)
            exchanges['binance'].create_market_buy_order('BTC/USDT', amount)
            exchanges[max_spread_ex].create_market_sell_order('BTC/KRW', get_exchange_rate() * amount * (btc_base + 0.01 * btc_base))
            profit = spread * amount * 20000
            send_telegram(f"Arbitrage 실행! {max_spread_ex} Spread {spread:.2f}% - 수익 +{profit:.0f}원")
        elif spread < -2.5:  # 역김프
            amount = 0.001
            exchanges[max_spread_ex].create_market_buy_order('BTC/KRW', get_exchange_rate() * amount * (btc_base - 0.01 * btc_base))
            exchanges['binance'].create_market_sell_order('BTC/USDT', amount)
            profit = abs(spread) * amount * 20000
            send_telegram(f"역김프 실행! {max_spread_ex} Spread {spread:.2f}% - 수익 +{profit:.0f}원")
        
        funding = get_funding_rate()
        if funding > 0.01:  # 꾸준 이자
            amount = 0.001
            exchanges['binance'].create_market_sell_order('BTC/USDT', amount, {'type': 'future'})
            profit = funding * amount * 20000 * 3  # 8시간 3회 가정
            send_telegram(f"Funding 실행! Rate {funding:.4f}% - 예상 이자 +{profit:.0f}원")
        
        time.sleep(60)
    except Exception as e:
        send_telegram(f"재시작: {e}")
        time.sleep(10)

if __name__ == '__main__':
    send_telegram("까망빠나나 시작! 안전 모드로 24/7 가동 중.")
    # 루프 실행
